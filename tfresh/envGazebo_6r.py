...
class GazeboWorld():
    ...
    def GetRewardAndTerminate_v1(self, t, is_training = True):
        terminate = False
        [x, y, o, v, w, r] = self.GetSelfState()
        [goalX, goalY] = self.GetGoal()
        obs = self.GetObstacle()  #(3, 6)
        crash = False
        for item in obs:
            obsDis = np.linalg.norm(np.array([item[1] - y, item[0] - x], dtype=np.float)) - r - item[5] + 0.17
            if obsDis <= 0:
                crash = True
                break
        s = np.array([goalY - y, goalX - x], dtype=np.float)
        self.distance = np.linalg.norm(s)
