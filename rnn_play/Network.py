...
class RNN_AC(nn.Module):
    ...
    def __init__(self, action_dim=2, state_dim=8, rnn_input_dim=6, 
    rnn_hidden_dim=64, hidden_sizes_ac=(64, 64), hidden_sizes_v=(64, 64), 
    activation=nn.ReLU, output_activation_v= nn.Identity, use_gpu=True, drop_p=0):
        ...
        self.pi = GaussianActor(obs_dim, action_dim, hidden_sizes_ac, activation, rnn_reader=rnn, use_gpu=use_gpu)
...
class GaussianActor(Actor):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, rnn_reader=None, use_gpu=False):
        super(GaussianActor, self).__init__()
        self.rnn_reader = rnn_reader
        self.use_gpu = use_gpu
        self.net_out=mlp([obs_dim] + list(hidden_sizes), activation, activation)
        ...
        if use_gpu:
            ...
            self.a_v_fc = self.a_v_fc.cuda()
            self.a_w_fc = self.a_w_fc.cuda()
        else:
    ...
    def _distribution(self, obs, std_factor=1):
        ...
        mean1 = sigmoid(self.a_v_fc(net_out))
        mean2 = tanh(self.a_w_fc(net_out))
