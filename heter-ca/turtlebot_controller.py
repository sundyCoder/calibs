...
class Turtlebot():
    def __init__(self, turtlebot, end):
        ...
        self.obstacle1_sub = rospy.Subscriber('robot1/odom', Point, self.obstacleCallBack1)
        self.obstacle2_sub = rospy.Subscriber('robot2/odom', Point, self.obstacleCallBack2)
        self.obstacle3_sub = rospy.Subscriber('robot3/odom', Point, self.obstacleCallBack3)
        self.obstacle4_sub = rospy.Subscriber('robot4/odom', Point, self.obstacleCallBack4)
        self.obstacle5_sub = rospy.Subscriber('robot5/odom', Point, self.obstacleCallBack5)
        self.obstacle6_sub = rospy.Subscriber('robot6/odom', Point, self.obstacleCallBack6)
        self.obstacle7_sub = rospy.Subscriber('robot7/odom', Point, self.obstacleCallBack7)
        self.obstacle8_sub = rospy.Subscriber('robot8/odom', Point, self.obstacleCallBack8)
        self.obstacle9_sub = rospy.Subscriber('robot9/odom', Point, self.obstacleCallBack9)
        self.obstacle10_sub = rospy.Subscriber('robot10/odom', Point, self.obstacleCallBack10)
        self.obstacle11_sub = rospy.Subscriber('robot11/odom', Point, self.obstacleCallBack11)
        self.obstacle12_sub = rospy.Subscriber('robot12/odom', Point, self.obstacleCallBack12)
    ...
    def getObstaclePos(self):
        ...
        if self.turtlebot != 'robot4':
            obstacle.append(processObs(self.obstacle4, 'burger'))
        if self.turtlebot != 'robot5':
            obstacle.append(processObs(self.obstacle5, 'waffle'))
        if self.turtlebot != 'robot6':
            obstacle.append(processObs(self.obstacle6, 'burger'))
        ...
        if self.turtlebot != 'robot9':
            obstacle.append(processObs(self.obstacle9, 'waffle'))
        if self.turtlebot != 'robot10':
            obstacle.append(processObs(self.obstacle10, 'burger'))
        if self.turtlebot != 'robot11':
            obstacle.append(processObs(self.obstacle11, 'waffle'))
        if self.turtlebot != 'robot0':
            obstacle.append(processObs(self.obstacle12, 'burger'))
        if len(obstacle) >= 1:
            return obstacle
        else:
    ...
    def runSingle(self, dis):
        ...
        with open(self.file, "w+") as file:
            ...
            while True:
                ...
                if sqrt((pos.x-self.goal[0])**2 + (pos.y-self.goal[1])**2) <= 0.4:
                    break
