...
# start_state = np.array([[1.5, -2.6, 2.08], [3., 0, np.pi], [1.5, 2.6, -2.08], [-1.5, 2.6, -1.04], [-3., 0., 0.], [-1.5, -2.6, 1.04]])
# end_state = np.array([[-1.5, 2.6], [-3, 0], [-1.5, -2.6], [1.5, -2.6], [3., 0], [1.5, 2.6]])

start_state = np.array([[1.5, -2.6, 2.08], [3., 0, np.pi]]) #, [1.5, 2.6, -2.08], [-1.5, 2.6, -1.04], [-3., 0., 0.], [-1.5, -2.6, 1.04]])
end_state = np.array([[-1.5, 2.6], [-3, 0]]) #, [-1.5, -2.6], [1.5, -2.6], [3., 0], [1.5, 2.6]])

all_radius = [0.32, 0.32, 0.32, 0.32, 0.32, 0.32]
...
class OptQCQP():
    def __init__(self, i, dt, num_robots):
        ...
        # self.neighbor_robots = np.array([[3., 0, 0.32], [1.5, 2.6, 0.32], [-1.5, 2.6, 0.32], [-3., 0., 0.32], [-1.5, -2.6, 0.32]])
        self.neighbor_robots = np.array([[3., 0, 0.32], [1.5, 2.6, 0.32]]) #, [-1.5, 2.6, 0.32], [-3., 0., 0.32], [-1.5, -2.6, 0.32]])
    ...
    def create_opt(self):
        ...
        self.robot_sta = [] # robot's state: s1, s2, s3
        for i in range(3):
            self.robot_sta += [self.m.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, name="s"+str(i))]
        ...
        for i in range(self.num_robots - 1): # neighbouring robots
            for j in range(3): # [x, y, r]
                self.neighbor_rob += [self.m.addVar(lb=-GRB.INFINITY, ub=GRB.INFINITY, name=coeff[j+3*i])]
        self.neighbor_rob = np.array(self.neighbor_rob).reshape((self.num_robots-1, 3))

        self.ep = [] # not initialized
        ...
        for i in range(len(self.neighbor_robots)-1): 
            ...
            self.m.addConstr((self.x_pred - self.neighbor_rob[i][0])**2 + (self.y_pred - self.neighbor_rob[i][1])**2 - (self.robot_radius + self.neighbor_rob[i][2])**2 >= 0, "c"+str(i))
        
        self.m.addConstr(2 * self.robot_sta[2] + self.w * self.dt == 2 * self.sin_cos_ref, "d1") # TODO: 2*\theta + w.dt = 2.ref
        self.m.addConstr(self.x_pred - self.robot_sta[0] == self.v * self.x_delta * self.dt, "d2") # 
        self.m.addConstr(self.y_pred - self.robot_sta[1] == self.v * self.y_delta * self.dt, "d3") # vy
        self.m.addGenConstrSin(self.sin_cos_ref, self.y_delta, "d4") # y_delta = sin(ref)
        self.m.addGenConstrCos(self.sin_cos_ref, self.x_delta, "d5") # x_delta = cos(ref)

        
        self.m.addConstrs((self.robot_sta[j] == self.robot_state[j] for j in range(3)), "current_state") # robot's initial state
        self.m.addConstrs((self.neighbor_rob[i][j] == self.neighbor_robots[i][j] for i in range(self.num_robots-1) for j in range(3)), "node") # neighbours' state
    ...
    def solve(self):
        ...
        if self.m.status == GRB.Status.OPTIMAL:
            ...
            print('Model is infeasible')
            return False, self.v_output, self.w_output
        elif self.m.status == GRB.Status.UNBOUNDED:
            print('Model is unbounded')
            return False, self.v_output, self.w_output
        else:
