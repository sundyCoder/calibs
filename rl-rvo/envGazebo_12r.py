...
class GazeboWorld():
    def __init__(self, index, num_env):
        ...
        self.odom_sub8 = rospy.Subscriber('robot8/odom', Odometry, self.obstacleCallBack8)
        self.odom_sub9 = rospy.Subscriber('robot9/odom', Odometry, self.obstacleCallBack9)
        self.odom_sub10 = rospy.Subscriber('robot10/odom', Odometry, self.obstacleCallBack10)
        self.odom_sub11 = rospy.Subscriber('robot11/odom', Odometry, self.obstacleCallBack11)
    ...
    def getObstaclePos(self):
        ...
        if self.index != 1:
            obstacle.append(processObs(self.obstacle1, 'burger'))
        if self.index != 2:
            obstacle.append(processObs(self.obstacle2, 'waffle'))
        if self.index != 3:
            obstacle.append(processObs(self.obstacle3, 'polycar'))
        if self.index != 4:
        ...
        if self.index != 8:
            obstacle.append(processObs(self.obstacle8, 'waffle'))
        if self.index != 9:
            obstacle.append(processObs(self.obstacle9, 'poycar'))
        if self.index != 10:
    ...
    def ttc_reward_cal(self, nbr_list):
        ...
        for nb in nbr_list:
            ...
            if 0 < obj_sense_dist < 5: # TODO: bugs: 1 for swarp, 5 for circle and random
