...
#from envGazebo_12r import GazeboWorld
...
def enjoy(rank, comm, env, policy, action_bound):
    ...
    dist_time_file = open("log.csv", "w+")
    pre_pose = np.array([state_frame[0], state_frame[1]])
    distance = 0
    ...
    while (not rospy.is_shutdown()):
        ...
        #########################################
        pos = np.array([env.GetSelfState()[0], env.GetSelfState()[1]])
        distance += sqrt((pos[0] - pre_pose[0])**2 + (pos[1] - pre_pose[1])**2)
        #########################################
        pre_pose = pos
        ...
        if terminal:
            ...
            ###########################
            print(rank,",", distance,",", rospy.get_rostime() - time_0)
            with open('log.csv', "a+") as file:
                csv_file = csv.writer(file)
                data = [rank, round(distance, 3), rospy.get_rostime() - time_0]
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        file = "policy_circle_8/Stage1_2000" # circle 8
        #file = "policy_random_8r/Stage1_800" # random_8
